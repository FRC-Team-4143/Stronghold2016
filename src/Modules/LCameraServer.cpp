...
void LCameraServer::FreeImageData(std::tuple<uint8_t*, unsigned int, unsigned int, bool> imageData) {
...
void LCameraServer::SetImageData(uint8_t* data, unsigned int size, unsigned int start, bool imaqData) {
	std::lock_guard<priority_recursive_mutex> lock(m_imageMutex);
...
void LCameraServer::Serve() {
	...
	if (setsockopt(sock, SOL_SOCKET, SO_REUSEADDR, &reuseAddr, sizeof(reuseAddr)) == -1) {
		wpi_setErrnoErrorWithContext("[LCameraServer] Error setting socket options");
		return;
	}
	...
	if (bind(sock, static_cast<sockaddr*>(static_cast<void*>(&address)), sizeof(address)) == -1) {
	...
	while (true) {
		...
		while (index < sizeof(req)) {
			int sizeRead = read(conn, static_cast<char*>(static_cast<void*>(&req)) + index, sizeof(req) - index);
			if (sizeRead == -1) {
		...
		while (true) {
			...
			if (write(conn, kMagicNumber, sizeof(kMagicNumber)) == -1) {
				wpi_setErrnoErrorWithContext("[LCameraServer] Error sending magic number");
