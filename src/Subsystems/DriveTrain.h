...
private:
  ...
  double
      thetaFL; // thetaFL, angle from Front Left Wheel to the center of rotation
  double thetaFR; // thetaFR, angle from Front Right Wheel to the center of
                  // rotation
  double
      thetaRL; // thetaRL, angle from Rear Left Wheel to the center of rotation
  double
      thetaRR; // thetaRR, angle from Rear Right Wheel to the center of rotation
  ...
  void SetSteerSetpoint(float FLSetPoint, float FRSetPoint, float RLSetPoint,
                        float RRSetPoint);
  void SetDriveSpeed(float FLSpeed, float FRSpeed, float RLSpeed,
                     float RRSpeed);
  void CurrentLimit();
...
public:
  ...
  DriveTrain();
  void SetWheelbase(float w, float x, float y); // sets dimensions of drive
                                                // train
  void SetMaxSpeed(float MaxSpeed); // sets the max speed of drive motors
  void SetOffsets(double FLOff, double FROff, double RLOff,
                  double RROff); // sets wheels to correct positions
  void InitDefaultCommand();     // sets default command
  void Crab(float twist, float y, float x,
            bool operatorControl); // default driving command
  // bool ResetTurns();
  void Lock();
  ...
  // bool unwindwheel(AnalogChannelVolt*, PIDController*, double offset, bool
  // output); //unwinds one wheel to fix wires (returns false if not done
  // unwinding)
  void GyroCrab(float desiredangle, float y, float x,
                bool operatorControl); // drive using gyro values
  void
  FieldCentricCrab(float twist, float y, float x,
                   bool operatorControl); // drive at 90 degree angle to field
  void PositionModeTwist(float desiredangle);
  void updateDistanceEncoders(); // updates the values of drive train variables
                                 // with distance encoder values
  double
  getDistanceEncodersValues(); // returns average value from driving encoders
